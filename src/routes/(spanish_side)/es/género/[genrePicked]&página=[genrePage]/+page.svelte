...
<div data-sveltekit-preload-data="touch" class="genre_movies_container">
    ...
    <div class="pages_list">
        {#if allPages > 500}
            <p>Página {theCurrentPage} de 500</p>
        {:else}
            <p>Página {theCurrentPage} de {allPages}</p>
        {/if}
    </div>
    ...
    <div class="pagination_container">
        ...
        <ul class="pagination_list">
            <li>
                {#if activePage === 1}
                    <a class="inactive" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=1`}>Primera</a>
                {:else}
                    <a class="page_item" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=1`}>Primera</a>
                {/if}
            </li>
            <li>
                {#if activePage === 1}
                    <a class="inactive" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=1`}>&#60;</a>
                {:else if activePage > 1}
                    <a class="page_item" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${activePage-1}`}>&#60;</a>
                {/if}
            </li>
            {#each resultsToDisplay as page}
                {#if page === activePage}
                    <li>
                        <a class="active" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${page}`}>{page}</a>
                    </li>
                {:else}
                    <li>
                        <a class="page_item" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${page}`}>{page}</a>
                    </li>
                {/if}
            {/each}
            <li>
                {#if activePage === finalPage}
                    <a class="inactive" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${activePage}`}>&gt;</a>
            ...
            <li>
                {#if activePage === finalPage}
                    <a class="inactive" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${finalPage}`}>Última</a>
                {:else}
                    <a class="page_item" href={`/es/g%C3%A9nero/${genreName}&p%C3%A1gina=${finalPage}`}>Última</a>
                {/if}
            </li>
        </ul>
    </div>
